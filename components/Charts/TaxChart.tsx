...
const CustomTooltip = ({ active, payload }: any) => {
  if (active && payload && payload.length) {
    ...
    const percentage = ((data.totalTaxPerWard / data.totalTax) * 100).toFixed(1)
    return (
      <div className="bg-background border border-border rounded-lg p-3 shadow-lg">
        <p className="font-semibold">{`Ward ${data.ward}`}</p>
        <p className="text-sm text-muted-foreground">Tax: ₹{data.totalTaxPerWard.toLocaleString()}</p>
  ...
  return (
    <Card className="w-full max-w-4xl">
      <CardHeader className="text-center">
        <CardTitle className="text-2xl font-bold">Tax Contributions by Ward</CardTitle>
        <CardDescription>Total Tax Collected: ₹{totalTax.toLocaleString()}</CardDescription>
      </CardHeader>
      <CardContent>
              ...
              <Legend
                ...
                formatter={(value, entry: any) => (
                  <span style={{ color: entry.color }}>
                    Ward {entry.payload.ward} (₹{entry.payload.totalTaxPerWard.toLocaleString()})
                  </span>
                )}
              />
            </PieChart>
        ...
        <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center p-3 bg-muted rounded-lg">
            <p className="text-sm text-muted-foreground"> Highest Contributing Ward </p>
            <p className="font-semibold">
              Ward {data.reduce((max, ward) => (ward.totalTaxPerWard > max.totalTaxPerWard ? ward : max)).ward}
            </p>
          </div>
          <div className="text-center p-3 bg-muted rounded-lg">
            <p className="text-sm text-muted-foreground">Lowest Contributing Ward</p>
            <p className="font-semibold">
              Ward {data.reduce((min, ward) => (ward.totalTaxPerWard < min.totalTaxPerWard ? ward : min)).ward}
            </p>
          </div>
          <div className="text-center p-3 bg-muted rounded-lg">
            <p className="text-sm text-muted-foreground">Average Tax</p>
            <p className="font-semibold"> ₹{Math.round(totalTax / data.length).toLocaleString()} </p>
          </div>
          <div className="text-center p-3 bg-muted rounded-lg">
